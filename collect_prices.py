...
# 운동과 무관한 상품 컷 (검색어 단어 충돌 방지: "부스터"→자동차 부품, "고스트"→캔들 등)
NEGATIVE_WORDS = ["자동차", "차량", "와이퍼", "오토바이", "캔들", "디퓨저", "홀더", "피규어",
                  "인형", "스티커", "붕대", "깁스", "의료용", "반려", "강아지", "고양이",
                  "트렁크", "테일박스", "쇼바", "본넷"]

# 카테고리별 필수 키워드 — 상품명에 하나는 들어 있어야 함 (영문은 대소문자 무시)
CATEGORY_WORDS = {
    "protein":    ["프로틴", "단백", "웨이", "게이너", "카제인"],
    "creatine":   ["크레아틴", "크레아퓨어", "크레알칼린"],
    "booster":    ["프리워크아웃", "워크아웃", "프리 운동", "펌핑", "헬스부스터", "헬스 부스터"],
    "guard":      ["보호대", "리스트랩", "스트랩", "벨트", "슬리브", "장갑", "그립", "테이프", "아대", "랩"],
    "supplement": ["오메가", "유산균", "프로바이오틱", "비타민", "카페인", "BCAA", "EAA", "아미노",
                   "글루타민", "아르기닌", "카르니틴", "ZMA", "아연", "마그네슘", "미네랄"],
    "chicken":    ["닭가슴살", "닭 가슴살", "치킨브레스트"],
}

# 카테고리별 최저 정상가(원) — 이보다 싸면 스틱/샘플/잡화류로 간주
MIN_PRICE = {"protein": 3000, "creatine": 2000, "booster": 5000, "guard": 2000, "supplement": 3000, "chicken": 1000, "etc": 500}
...
KNOWN_BRANDS = [
    ...
    "베어그립", "험블", "렙스", "인저", "sbd", "맥데이비드", "잠스트", "하빈저",
    "블랙다이아몬드", "하이뮨", "노르딕내추럴스", "락토핏", "컬처렐", "자로우",
    "센트룸", "솔가", "닥터스베스트",
    "아임닭", "허닭", "잇메이트", "맛있닭", "미트리", "한끼통살", "닭신", "하림",
]
...
def validate(query, cat, name, price):
    ...
    for w in NEGATIVE_WORDS:
        if w in n:
            return False, f"운동 무관 상품({w})"
    req = CATEGORY_WORDS.get(cat)
    if req:
        nu = n.upper()
        if not any(w.upper() in nu for w in req):
            return False, "카테고리 키워드 없음"
    try:
